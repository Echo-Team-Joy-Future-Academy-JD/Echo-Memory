#!/bin/bash
# Dynamic SpatialVID inference: legacy VideoSSM hybrid.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common_env.sh"
: "${CKPT:?ERROR: set CKPT to your dynamic videossm_hybrid checkpoint path}"
OUTPUT="${INFER_OUTPUT_ROOT}/dyn_videossm_hybrid_$(date +%Y%m%d_%H%M%S).mp4"
EXTRA_ARGS=()
...
python inference/unified_inference.py \
  --ckpt "${CKPT}" --memory_type videossm_hybrid --base_model "${WAN_BASE_MODEL}" \
