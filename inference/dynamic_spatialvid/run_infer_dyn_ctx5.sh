#!/bin/bash
# Dynamic SpatialVID inference: Context K=5.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common_env.sh"
: "${CKPT:?ERROR: set CKPT to your dynamic ctx5 checkpoint path}"
OUTPUT="${INFER_OUTPUT_ROOT}/dyn_ctx5_$(date +%Y%m%d_%H%M%S).mp4"
EXTRA_ARGS=()
...
python inference/unified_inference.py \
  --ckpt "${CKPT}" --memory_type context_k5 --base_model "${WAN_BASE_MODEL}" \
