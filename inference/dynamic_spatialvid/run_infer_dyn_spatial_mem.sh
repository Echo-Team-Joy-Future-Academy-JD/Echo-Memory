#!/bin/bash
# Dynamic SpatialVID inference: Spatial Memory.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common_env.sh"
: "${CKPT:?ERROR: set CKPT to your dynamic spatial_mem checkpoint path}"
OUTPUT="${INFER_OUTPUT_ROOT}/dyn_spatial_mem_$(date +%Y%m%d_%H%M%S).mp4"
EXTRA_ARGS=()
...
python inference/unified_inference.py \
  --ckpt "${CKPT}" --memory_type spatial_mem --base_model "${WAN_BASE_MODEL}" \
