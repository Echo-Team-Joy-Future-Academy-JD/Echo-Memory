#!/bin/bash
# Inference: No memory baseline (I2V floor).
# Corresponds to: train/memory_baselines_basic/run_ablation_no_memory_baseline_two_chunk.sh
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common_env.sh"

: "${CKPT:?ERROR: set CKPT to your no-memory checkpoint path}"
OUTPUT="${INFER_OUTPUT_ROOT}/no_memory_$(date +%Y%m%d_%H%M%S).mp4"
