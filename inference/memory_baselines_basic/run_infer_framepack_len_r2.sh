#!/bin/bash
# Inference: framepack_len_r2
# Corresponds to: train/memory_baselines_basic/run_ablation_framepack_len_r2_two_chunk.sh
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common_env.sh"

: "${CKPT:?ERROR: set CKPT to your framepack_len_r2 checkpoint path}"
OUTPUT="${INFER_OUTPUT_ROOT}/framepack_len_r2_$(date +%Y%m%d_%H%M%S).mp4"
