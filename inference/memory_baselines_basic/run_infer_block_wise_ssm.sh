#!/bin/bash
# Inference: block_wise_ssm
# Corresponds to: train/memory_baselines_basic/run_ablation_block_wise_ssm_two_chunk.sh
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common_env.sh"

: "${CKPT:?ERROR: set CKPT to your block_wise_ssm checkpoint path}"
OUTPUT="${INFER_OUTPUT_ROOT}/block_wise_ssm_$(date +%Y%m%d_%H%M%S).mp4"
