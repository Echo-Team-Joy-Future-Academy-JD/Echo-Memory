#!/bin/bash
# Inference: Context K=5
# Corresponds to: train/context_learning/run_pre_qkv_ctx5.sh
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common_env.sh"

: "${CKPT:?ERROR: set CKPT to your context_k5 checkpoint path}"
OUTPUT="${INFER_OUTPUT_ROOT}/context_k5_$(date +%Y%m%d_%H%M%S).mp4"
