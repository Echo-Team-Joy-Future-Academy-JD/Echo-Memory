#!/bin/bash
# Dynamic SpatialVID: Spatial Memory baseline.
set -euo pipefail
...
accelerate launch src/model_training/train.py \
  ...
  --wandb_run_name "${WANDB_RUN_NAME:-dyn_spatialvid_spatial_mem}" \
  ...
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/dyn_spatialvid_spatial_mem_$(date +%Y%m%d_%H%M%S).log"
