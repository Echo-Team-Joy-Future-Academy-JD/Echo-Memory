#!/bin/bash
# Dynamic SpatialVID: Context K=20 baseline.
set -euo pipefail
...
accelerate launch src/model_training/train.py \
  ...
  --output_path "${output_base}_ctx20" --trainable_models dit --ckpt_interval "${CKPT_INTERVAL:-1000}" --save_full_model \
  --wandb_run_name "${WANDB_RUN_NAME:-dyn_spatialvid_ctx20}" \
  ...
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/dyn_spatialvid_ctx20_$(date +%Y%m%d_%H%M%S).log"
