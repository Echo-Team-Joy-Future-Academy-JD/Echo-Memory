#!/bin/bash
# pre_qkv 系列：action 在 norm1 之后、3D self-attn 之前与 frame 交互。ctx=5，每帧单独 VAE、ctx action 按条数输入，target action stride-4
...
accelerate launch src/model_training/train.py \
  ...
  --output_path "${output_base}_merged_cam_ctx_5_noise_5_atomic_cam_inject_pre_qkv_per_frame_vae" --trainable_models dit --ckpt_interval 5000 --save_full_model \
  --wandb_run_name "exp1_4_4_ctx_5_noise_5_atomic_cam_inject_pre_qkv_per_frame_vae" \
  ...
  2>&1 | tee "${LOG_DIR}/exp1_4_4_ctx_5_pre_qkv_per_frame_vae_$(date +%Y%m%d_%H%M%S).log"
