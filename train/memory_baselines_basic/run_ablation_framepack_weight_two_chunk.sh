#!/bin/bash
# Ablation: FramePack-Weight + 2-chunk train monitor.
set -euo pipefail
...
accelerate launch src/model_training/train.py \
  ...
  --output_path "${output_base}_abl_framepack_weight_two_chunk" --trainable_models dit --ckpt_interval "${CKPT_INTERVAL:-1000}" --save_full_model \
  --wandb_run_name "abl_framepack_weight_two_chunk" \
  ...
  2>&1 | tee "${LOG_DIR}/abl_framepack_weight_two_chunk_$(date +%Y%m%d_%H%M%S).log"
