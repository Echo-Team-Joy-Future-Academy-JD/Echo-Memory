#!/bin/bash
# Ablation: SpatialGridMemory + dedicated cross-attention read-out + 2-chunk monitor.
set -euo pipefail
...
accelerate launch src/model_training/train.py \
  ...
  --output_path "${output_base}_abl_spatial_cross_attn_readout_two_chunk" --trainable_models dit --ckpt_interval "${CKPT_INTERVAL:-1000}" --save_full_model \
  --wandb_run_name "abl_spatial_cross_attn_readout_two_chunk" \
  ...
  --use_spatial_memory --spatial_memory_tokens 64 --spatial_memory_inject_mode cross_attn_readout \
  --timestep_shift "${TIMESTEP_SHIFT:-15}" \
  "${SAMPLING_TWO_CHUNK_FLAGS[@]}" \
  2>&1 | tee "${LOG_DIR}/abl_spatial_cross_attn_readout_two_chunk_$(date +%Y%m%d_%H%M%S).log"
